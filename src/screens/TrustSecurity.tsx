...
import { ChevronLeft, Landmark, ShieldCheck, Lock, HeartPulse, ScanFace } from "lucide-react";
      ...
      <motion.div variants={stagger} initial="hidden" animate="show" className="no-scrollbar flex-1 overflow-y-auto px-5 pb-10 pt-4">
        ...
        <motion.div variants={item} className="relative overflow-hidden rounded-[16px] border border-teal/25 bg-teal/8 p-5">
          <div className="flex items-center gap-3">
            <span className="flex h-12 w-12 flex-none items-center justify-center rounded-full bg-teal/15 text-teal-2"><ShieldCheck size={24} strokeWidth={2.2} /></span>
            <div>
          ...
          {SECTIONS.map((s) => (
            <motion.div key={s.title} variants={item} className="rounded-[14px] border border-border bg-surface p-4">
              <div className="flex items-center gap-2.5">
                <span className="flex h-9 w-9 flex-none items-center justify-center rounded-[10px] bg-surface-2 text-teal-2"><s.icon size={18} strokeWidth={2.1} /></span>
