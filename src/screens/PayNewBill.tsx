...
import { Zap, Droplet, Wifi, Flame, Phone, Check, ChevronRight, Loader, type LucideIcon } from "lucide-react";
...
interface Cat { id: string; label: string; icon: LucideIcon; billers: string[]; due: number }
export const CATEGORIES: Cat[] = [
  { id: "electricity", label: "Electricity", icon: Zap, billers: ["Austin Energy", "City of Austin Utilities", "CenterPoint Energy", "Oncor Electric"], due: 412.6 },
  { id: "water", label: "Water", icon: Droplet, billers: ["Austin Water", "City of Austin Utilities"], due: 98.4 },
  { id: "internet", label: "Internet", icon: Wifi, billers: ["Google Fiber", "AT&T Fiber", "Spectrum Business"], due: 89.0 },
  { id: "gas", label: "Gas", icon: Flame, billers: ["Texas Gas Service", "Atmos Energy"], due: 64.2 },
  { id: "phone", label: "Phone", icon: Phone, billers: ["AT&T Business", "Verizon", "T-Mobile"], due: 220.0 },
];
          ...
          <motion.div key="cat" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.22 }} className="grid grid-cols-3 gap-2.5">
            {CATEGORIES.map((c) => (
              <button key={c.id} onClick={() => { haptic("tap"); setCat(c); setStep("biller"); }} className="flex flex-col items-center gap-2 rounded-[10px] border border-border bg-surface-2 py-4">
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-teal/12 text-teal-2"><c.icon size={20} strokeWidth={2} /></span>
          ...
          <motion.div key="biller" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.22 }} className="rounded-[10px] border border-border bg-surface-2 px-3.5">
            {cat.billers.map((b, i) => (
              <button key={b} onClick={() => { haptic("tap"); setBiller(b); setStep("account"); }} className={"flex w-full items-center gap-3 py-3.5 text-left " + (i > 0 ? "border-t border-border-soft" : "")}>
                <span className="flex h-9 w-9 items-center justify-center rounded-[9px] bg-surface text-teal-2"><cat.icon size={16} strokeWidth={2} /></span>
        ...
        {step === "account" && cat && (
          <motion.div key="acct" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.22 }} className="space-y-4">
            <div className="flex items-center gap-2.5 rounded-[10px] border border-border bg-surface-2 p-3">
              <span className="flex h-9 w-9 flex-none items-center justify-center rounded-[9px] bg-teal/12 text-teal-2"><cat.icon size={17} strokeWidth={2} /></span>
            ...
            <NeoPopButton onClick={fetchBill} className="w-full" faceClassName="px-5 py-4 text-[15px] font-medium">Fetch my bill <ChevronRight size={18} strokeWidth={2.2} /></NeoPopButton>
          </motion.div>
        )}
        ...
        {step === "review" && cat && (
          <motion.div key="review" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.22 }}>
            <div className="flex items-center gap-3 rounded-[10px] border border-border bg-surface-2 p-3.5">
              <span className="flex h-11 w-11 flex-none items-center justify-center rounded-[9px] bg-teal/12 text-teal-2"><cat.icon size={20} strokeWidth={2} /></span>
            ...
            <NeoPopButton onClick={pay} className="mt-5 w-full" faceClassName="px-5 py-4 text-[15px] font-medium">Pay {money(cat.due)} <ChevronRight size={18} strokeWidth={2.2} /></NeoPopButton>
          </motion.div>
        )}
