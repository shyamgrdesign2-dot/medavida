...
// ReactBits GradientWaves (https://reactbits.dev/backgrounds/gradient-waves),
// adapted to TS + themed to Zeva teal. WebGL raymarched plasma waves.
...
float raymarch(vec3 pos, vec3 dir, vec2 freq, vec4 tc){
  float dist=0.0;
  for(int i=0;i<128;i++){ if(float(i)>=uSteps) break; float ds=plasma(pos+dist*dir,freq,tc);
    if(abs(ds)<0.1) break; dist+=0.9*ds; if(!(abs(dist)<MAX_DIST)) return MAX_DIST; }
  return dist;
}
void main(){
  ...
  if(uEnableMouse){ float yaw=(uMouse.x-0.5)*uParallax*0.4; float pitch=(uMouse.y-0.5)*uParallax*0.4;
  ...
  vec3 col=mix(uHorizonColor,body,t); col*=uBrightness; col=clamp(col,0.0,1.0);
...
export function GradientWaves({
  ...
  const ctxRef = useRef<{ program: any } | null>(null);
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uSteps: { value: detailToSteps(detail) }, uBrightness: { value: brightness }, uOpacity: { value: opacity },
    ...
    const onMove = (e: PointerEvent) => { const r = canvas.getBoundingClientRect(); tgt[0] = (e.clientX - r.left) / r.width; tgt[1] = 1 - (e.clientY - r.top) / r.height; };
    const onLeave = () => { tgt[0] = 0.5; tgt[1] = 0.5; };
    canvas.addEventListener("pointermove", onMove); canvas.addEventListener("pointerleave", onLeave);

    let raf = 0; let vis = true; let pageVis = !document.hidden; const t0 = performance.now();
    const loop = (t: number) => {
      program.uniforms.iTime.value = (t - t0) * 0.001;
      const tx = enableMouseRef.current ? tgt[0] : 0.5, ty = enableMouseRef.current ? tgt[1] : 0.5;
      cur[0] += 0.05 * (tx - cur[0]); cur[1] += 0.05 * (ty - cur[1]);
    ...
    const start = () => { if (vis && pageVis && raf === 0) raf = requestAnimationFrame(loop); };
    const stop = () => { if (raf !== 0) { cancelAnimationFrame(raf); raf = 0; } };
    const io = new IntersectionObserver(([e]) => { vis = e.isIntersecting; vis ? start() : stop(); }, { threshold: 0 });
    io.observe(container);
    const onVisibility = () => { pageVis = !document.hidden; pageVis ? start() : stop(); };
  ...
  // live-update uniforms on prop change
  useEffect(() => {
    const u = ctxRef.current?.program?.uniforms; if (!u) return;
    enableMouseRef.current = mouseInteraction;
    u.uSpeed.value = speed; u.uAmplitude.value = amplitude; u.uWaveScale.value = waveScale; u.uWaveRatio.value = waveRatio;
    u.uSwell.value = swell; u.uTurbulence.value = turbulence; u.uTilt.value = tilt; u.uZoom.value = zoom;
    u.uHeight.value = height; u.uFogDepth.value = fogDepth; u.uSteps.value = detailToSteps(detail);
    u.uBrightness.value = brightness; u.uOpacity.value = opacity; u.uGrain.value = grain ? 1 : 0;
    u.uGrainIntensity.value = grainIntensity; u.uParallax.value = parallaxStrength; u.uEnableMouse.value = mouseInteraction;
